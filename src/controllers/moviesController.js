...
const moviesController = {
  ...
  edit: function (req, res) {
    ...
    let promMovies = db.Movie.findByPk(movieId, {
  ...
  update: function (req, res) {
    let movieId = req.params.id;
 console.log("se mandooo",movieId)
    db.Movie.update(
      {
        title: req.body.title,
        rating: req.body.rating,
      ...
      .then((result) => res.json({ mag: "movie actualizada", result, movie : req.body}))
      .catch((error) => res.json(error));
  },
  ...
  destroy: function (req, res) {
    ...
    Movies.destroy({ where: { id: movieId }, force: true }) // force: true es para asegurar que se ejecute la acción
      .then((result) => res.json({result, movieID}))
      .catch((error) => res.send(error));
  },
};
